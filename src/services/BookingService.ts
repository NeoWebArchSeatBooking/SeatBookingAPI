...
import { BookedSeat, Booking, BookingRequest, SeatInfo, SeatingRequest } from "../models"
...
class BookingService{
    ...
    public async bookSeat(req: BookingRequest): Promise<void>{
        const seats = await bookingDataAccess.getBookedSeats(req);
        if(seats.findIndex((seat)=> seat === req.seatId) > -1){
            logger.info(`seat ${req.seatId} not available`)
            throw new ConflictError("seat not available now, pick some other seat");
        }
        const hasBooked = await bookingDataAccess.getBookedSeatsByUserAndDate(req.userId,req.date)
        if(hasBooked) throw new ConflictError("user already has booked a seat on the date");
